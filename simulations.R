...
mu[[1]] = rnorm(1, mean = mean_density, sd = sigma_gen[1])
alpha[[1]] = rnorm(n, mean = 0, sd = sigma_gen[2])
beta[[1]] = rnorm(n, mean = 0, sd = sigma_gen[2])
u[[1]] = mvrnorm(n, mu = c(0,0), Sigma = matrix(data = c(sigma_gen[3],sigma_gen[4],sigma_gen[4],sigma_gen[3]), nrow = 2))
v[[1]] = mvrnorm(n, mu = c(0,0), Sigma = matrix(data = c(sigma_gen[3],sigma_gen[4],sigma_gen[4],sigma_gen[3]), nrow = 2))

for(period in 2:periods) {
  mu[[period]] = mu[[period-1]] + rnorm(1, mean = 0, sd = sigma_walk[1])
  alpha[[period]] = alpha[[period-1]] + rnorm(n, mean = 0, sd = sigma_walk[2])
  beta[[period]] = beta[[period-1]] + rnorm(n, mean = 0, sd = sigma_walk[2])
  u[[period]] = u[[period-1]] + mvrnorm(n, mu = c(0,0), Sigma = matrix(data = c(sigma_walk[3],sigma_walk[4],sigma_walk[4],sigma_walk[3]), nrow = 2))
  v[[period]] = v[[period-1]] + mvrnorm(n, mu = c(0,0), Sigma = matrix(data = c(sigma_walk[3],sigma_walk[4],sigma_walk[4],sigma_walk[3]), nrow = 2))
}
...
epsilon = 3e-03
ldim = 2
sample_prop_0 = 1 #proportion of 0s to sample (in section 3, ran with both 0.025 and 1)
...
for(period in 1:periods) {
  ...
  while(TRUE) {
    ...
    print(paste("Period: ",period,", Iter: ",counter,", Time: ",format(round(difftime(Sys.time(),start,units="mins"),3),nsmall=4),", Rel Tol: ", format(criteria, nsmall=4),", Latent Cor: ",format(cor(as.numeric(u[[period]] %*% t(v[[period]])), as.numeric(q_u_mean %*% t(q_v_mean))),nsmall = 4),sep=""))
